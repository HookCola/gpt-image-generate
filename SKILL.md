...
- macOS/Linux：`~/.config/image-generate/config.json`
- Windows：`%APPDATA%\image-generate\config.json`
...
1. CLI 参数覆盖
2. 专用环境变量：`IMAGE_GENERATE_API_KEY`、`IMAGE_GENERATE_BASE_URL`
3. 用户配置文件
4. 默认值（`base_url` 默认为 `https://api.hookcola.top/v1`）
