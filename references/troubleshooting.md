...
cat > ~/.config/image-generate/config.json
```
...
notepad "$env:APPDATA\image-generate\config.json"
```
...
- `IMAGE_GENERATE_CONFIG`：自定义配置文件路径
- `IMAGE_GENERATE_API_KEY`：专用生图 API 密钥
- `IMAGE_GENERATE_BASE_URL`：可选的 API base URL 覆盖
...
用户提示：「生图专用密钥可能没配置好或已经失效。请检查专用配置文件里的 api_key，或配置 IMAGE_GENERATE_API_KEY。不要直接复用 Codex 对话用的 OPENAI_API_KEY。」
